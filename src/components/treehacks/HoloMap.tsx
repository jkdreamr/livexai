  ...
  return (
    <div className="relative flex h-full w-full items-center justify-center" style={{ perspective: 900 }}>
      <div
        ...
        style={{ background: "radial-gradient(120% 80% at 50% 100%, rgba(47,224,138,0.14), transparent 70%)" }}
      />
      <div style={{ transform: "rotateX(46deg) rotateZ(0deg)", transformStyle: "preserve-3d" }} className="w-full max-w-md">
        <svg viewBox="0 0 360 214" className="w-full drop-shadow-[0_0_20px_rgba(47,224,138,0.22)]" aria-label="Venue map">
          {/* floor grid */}
          <g stroke="rgba(47,224,138,0.14)" strokeWidth="0.5">
          ...
          {ROOMS.map((r) => {
            ...
            return (
              <g key={r.id}>
                <rect x={r.x} y={r.y + 4} width={r.w} height={r.h} rx="4" fill="rgba(47,224,138,0.07)" />
                <rect
                  x={r.x}
                  y={r.y}
                  width={r.w}
                  height={r.h}
                  rx="4"
                  fill={on ? "rgba(47,224,138,0.16)" : "rgba(10,20,16,0.6)"}
                  stroke={on ? "#2fe08a" : "rgba(130,220,180,0.45)"}
                  strokeWidth={on ? "1.6" : "0.8"}
                />
                <text x={r.x + r.w / 2} y={r.y + r.h / 2 + 3} textAnchor="middle" fontSize="9" fill={on ? "#d8fff0" : "rgba(205,240,222,0.7)"} className="font-mono">
                  {r.name}
                </text>
              </g>
            );
          ...
          {t && (
            <motion.path
              ...
              style={{ filter: "drop-shadow(0 0 4px #2fe08a)" }}
            />
          )}

          {/* you are here */}
          <circle cx={YOU.x} cy={YOU.y} r="5" fill="#9d7bff" />
          <motion.circle
            cx={YOU.x}
            cy={YOU.y}
            r="5"
          ...
          {t && (
            <g>
              <line x1={t.x} y1={t.y} x2={t.x} y2={t.y - 22} stroke="#2fe08a" strokeWidth="1.4" />
              <circle cx={t.x} cy={t.y - 24} r="4" fill="#2fe08a" style={{ filter: "drop-shadow(0 0 6px #2fe08a)" }} />
            </g>
          )}
        </svg>
      </div>
