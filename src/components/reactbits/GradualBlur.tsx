...
const getGradientDirection = (position: string) =>
  (({ top: "to top", bottom: "to bottom", left: "to left", right: "to right" } as any)[
    position
  ] || "to bottom");
...
function GradualBlur(props: any) {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      let blurValue;
      if (config.exponential) blurValue = Math.pow(2, progress * 4) * 0.0625 * currentStrength;
      else blurValue = 0.0625 * (progress * config.divCount + 1) * currentStrength;
